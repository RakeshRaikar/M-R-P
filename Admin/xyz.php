  ...
  if (isset($_POST['selected_rows']) && is_array($_POST['selected_rows'])) {
    ...
    foreach ($selectedRows as $rowId) {
        $rowId = intval($rowId);
            if($status == 'Rejected')
                {
                    $query = "UPDATE invoice_data44 SET STATUS=?,slot='-' WHERE id=?";
            ...
            if($status == 'Approved')
            {
                ...
                while ($row = mysqli_fetch_assoc($result1))
                {
                    ...
                    if($weight <=100)
                    {
                        ...
                        $stmt1->bind_param("sd", $rowId,$row['slotid']);
                        if (!$stmt1) {
                            ...
                            { 
                                ...
                                $stmt->bind_param("sdi", $status, $slot1, $rowId);
                                if ($stmt->execute()) {
                            ...
                            if ($row['slotstatus']==0)
                            {
                                ...
                                { 
                                    ...
                                    if ($loop==1){
                                        ...
                                        $stmt = $conn->prepare("UPDATE invoice_data44 SET status=?,slot=? WHERE id=?");
                                        $stmt->bind_param("ssi", $status, $slot1, $rowId);
                                        if ($stmt->execute()) {
                                            echo "Status updated successfully";
                                        } else {
                                            echo "Status update failed: " . $stmt->error;
